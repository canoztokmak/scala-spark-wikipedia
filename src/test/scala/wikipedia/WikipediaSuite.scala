...
class WikipediaSuite extends FunSuite with BeforeAndAfterAll {
  ...
  test("rankLangs") {
    ...
    val rdd = sc.textFile(WikipediaData.filePath).map(WikipediaData.parse).cache()
    val ranked = rankLangs(WikipediaRanking.langs, rdd)
    println("1", ranked)
    assert(true)
  }

  test("rankLangsUsingIndex") {
    import WikipediaRanking._

    val rdd = sc.textFile(WikipediaData.filePath).map(WikipediaData.parse).cache()
    val index = makeIndex(WikipediaRanking.langs, rdd)
    val ranked = rankLangsUsingIndex(index)
    println("2", ranked)
    assert(true)
  }

  test("rankLangsReduceByKey") {
    import WikipediaRanking._

    val rdd = sc.textFile(WikipediaData.filePath).map(WikipediaData.parse).cache()
    val ranked = rankLangsReduceByKey(WikipediaRanking.langs, rdd)
    println("3", ranked)
    assert(true)
  }
}
